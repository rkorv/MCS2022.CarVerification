    ...
    def __init__(
        ...
        if expansion_factor != 1:
            self.expand_conv = nn.Conv2d(
                in_channels=in_channels, out_channels=exp_channels, kernel_size=1, bias=bias
            )
        ...
        self.project_conv = nn.Conv2d(
            in_channels=exp_channels, out_channels=out_channels, kernel_size=1, bias=bias
        )
...
class EfficientNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.flatten(self.avpool(x), 1)


if __name__ == "__main__":
    model = EfficientNetV2("m", n_classes=1024, in_spatial_shape=(400, 400), pretrained=True)

    img = torch.zeros((1, 3, 300, 300))

    o = model(torch.zeros((1, 3, 300, 300)))
    print(o.shape)
